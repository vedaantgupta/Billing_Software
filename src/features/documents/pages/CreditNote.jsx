...
import '@/features/documents/styles/PurchaseInvoice.css'; // Using the Proforma Invoice styling
import '@/features/products/styles/product-table.css';

// ─── Main Component ──────────────────────────────────────────────────────────

const DOC_TYPES = ['Credit Note', 'Sales Return', 'Price Difference', 'Discount Credit Note'];
const CN_TYPES = ['Regular', 'Correction', 'Return', 'Debit Adjustment'];
...
const CreditNote = () => {
   ...
   const handleSave = async (print = false) => {
      if (!user?.id) return;
      if (!doc.cnDetail.cnNo) { alert('Please enter Credit Note No.'); return; }
      ...
      try {
         ...
         logActivity(id ? `Updated ${doc.docType} #${fullNo}` : `Created ${doc.docType} #${fullNo}`, user.id, user.username);
         ...
         alert(`Failed to save ${doc.docType}.`);
      } finally {
   ...
   if (loading && user) {
      return <div className="pi-page" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading Credit Note Builder...</div>;
   }
      ...
      <div className="pi-page">
         ...
         <div className="pi-header">
            <div className="pi-header-left">
               <div className="pi-badge" style={{ background: '#ef4444', color: 'white' }}>
                  📉 CREDIT NOTE
               </div>
               <div>
                  <div className="pi-title">Credit Note Builder</div>
                  <div className="pi-subtitle">
                     {id ? `Editing • CN-${doc.cnDetail.cnNo}` : 'Create a new credit note record'}
                  </div>
               </div>
            </div>
         ...
         {/* ── Top Two-Column Grid: Customer + CN Details ────── */}
               ...
               <div className="pi-card-body">
                  ...
                  <div className="pi-field-row" style={{ gridTemplateColumns: '130px 1fr' }}>
                     ...
                     <div className="pi-ms-row">
                        ...
                        <button className="pi-ms-add-btn" onClick={() => setIsAddContactModalOpen(true)}>+</button>
                     </div>
                  </div>
                  ...
                  <div className="pi-field-row" style={{ gridTemplateColumns: '130px 1fr' }}>
                     ...
                     <input className="pi-input" placeholder="GSTIN / PAN" value={doc.customerInfo.gstinPan} onChange={e => handleNested('customerInfo', 'gstinPan', e.target.value.toUpperCase())} />
                  </div>
            ...
            <div className="pi-card">
               <div className="pi-card-header" style={{ background: 'white' }}>
                  <div className="pi-card-header-left">
                     ...
                     <div>
                        <div className="pi-card-title">Credit Note Detail</div>
                        <div className="pi-card-subtitle">Tracking & Linkage</div>
                     </div>
                  </div>
                  <button className="pi-reset-btn" onClick={() => setDoc({ ...doc, cnDetail: { ...doc.cnDetail, cnNo: '1', date: todayIso() } })}><RotateCcw size={16} /></button>
               </div>
               <div className="pi-card-body">

                  {/* Row 1: Doc Type | C.N. Type */}
                  <div className="pi-field-row two-col" style={{ gridTemplateColumns: '100px 1fr 80px 1fr' }}>
                     <label className="pi-label">Doc. Type</label>
                     <select className="pi-select" value={doc.cnDetail.type} onChange={e => handleNested('cnDetail', 'type', e.target.value)}>
                        {DOC_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                     </select>
                     <label className="pi-label" style={{ textAlign: 'right' }}>C.N. Type</label>
                     <select className="pi-select" value={doc.cnDetail.cnType} onChange={e => handleNested('cnDetail', 'cnType', e.target.value)}>
                        {CN_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                     </select>
                  </div>

                  {/* Row 2: C.N. No | C.N. Date */}
                  <div className="pi-invoice-no-row" style={{ gridTemplateColumns: '100px 1fr' }}>
                     <label className="pi-label">C.N. No.<span className="req">*</span></label>
                     <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        {/* Grouped no-gap prefix/number/postfix */}
                        <div style={{ display: 'flex', alignItems: 'center', border: '1.5px solid #e2e8f0', borderRadius: '8px', overflow: 'hidden' }}>
                           <input
                              className="pi-no-input"
                              style={{ width: '36px', textAlign: 'center', borderRadius: 0, border: 'none', borderRight: '1px solid #e2e8f0', padding: '0.42rem 0.2rem', boxSizing: 'border-box' }}
                              placeholder="Pr"
                              value={doc.docPrefix}
                           ...
                           <input
                              className="pi-no-input"
                              style={{ width: '44px', textAlign: 'center', borderRadius: 0, border: 'none', borderRight: '1px solid #e2e8f0', padding: '0.42rem 0.2rem', boxSizing: 'border-box' }}
                              value={doc.cnDetail.cnNo}
                              onChange={e => handleNested('cnDetail', 'cnNo', e.target.value)}
                           />
                           <input
                              className="pi-no-input"
                              style={{ width: '60px', textAlign: 'center', borderRadius: 0, border: 'none', padding: '0.42rem 0.2rem', boxSizing: 'border-box' }}
                              placeholder="Po"
                              value={doc.docPostfix}
                              onChange={e => setDoc({ ...doc, docPostfix: e.target.value })}
                           />
                        </div>
                        <div className="pi-date-group" style={{ gap: '0.25rem' }}>
                           <span className="pi-date-label" style={{ fontSize: '0.75rem', whiteSpace: 'nowrap' }}>C.N. Date<span className="req">*</span></span>
                           <input type="date" className="pi-date-input" style={{ width: '135px', fontSize: '0.75rem', padding: '0.42rem 0.3rem' }} value={doc.cnDetail.date} onChange={e => handleNested('cnDetail', 'date', e.target.value)} />
                        </div>
                     </div>
                  </div>

                  {/* Row 3: Invoice No ("Against Invoice No") */}
                  <div className="pi-field-row" style={{ gridTemplateColumns: '100px 1fr' }}>
                     <label className="pi-label">Invoice No.<span className="req">*</span></label>
                     <div>
                        <input className="pi-input" style={{ color: '#ef4444' }} placeholder="Against Invoice No" value={doc.cnDetail.invoiceNo} onChange={e => handleNested('cnDetail', 'invoiceNo', e.target.value)} />
                        <div className="pi-label-italic" style={{ marginTop: '0.2rem', color: '#94a3b8', fontSize: '0.65rem' }}>Please invoice number with prefix & postfix.</div>
                     </div>
                  </div>

                  {/* Row 4: Invoice Date */}
                  <div className="pi-field-row" style={{ gridTemplateColumns: '100px 1fr' }}>
                     <label className="pi-label">Invoice Date</label>
                     <input type="date" className="pi-date-input" style={{ width: '150px' }} value={doc.cnDetail.invoiceDate} onChange={e => handleNested('cnDetail', 'invoiceDate', e.target.value)} />
                  </div>

                  {/* Row 5: Challan No | Challan Date */}
                  <div className="pi-field-row two-col" style={{ gridTemplateColumns: '100px 1fr 100px 1fr' }}>
                     <label className="pi-label">Challan No.</label>
                     <input className="pi-input" placeholder="Challan No." value={doc.cnDetail.challanNo} onChange={e => handleNested('cnDetail', 'challanNo', e.target.value)} />
                     <label className="pi-label" style={{ textAlign: 'right' }}>Challan Date</label>
                     <input type="date" className="pi-date-input" style={{ width: '135px' }} value={doc.cnDetail.challanDate} onChange={e => handleNested('cnDetail', 'challanDate', e.target.value)} />
                  </div>

                  {/* Row 6: L.R. No | E-Way No */}
                  <div className="pi-field-row two-col" style={{ gridTemplateColumns: '100px 1fr 100px 1fr' }}>
                     <label className="pi-label">L.R. No.</label>
                     <input className="pi-input" placeholder="L.R. No." value={doc.cnDetail.lrNo} onChange={e => handleNested('cnDetail', 'lrNo', e.target.value)} />
                     <label className="pi-label" style={{ textAlign: 'right' }}>E-Way No.</label>
                     <input className="pi-input" placeholder="E-Way No." value={doc.cnDetail.ewayNo} onChange={e => handleNested('cnDetail', 'ewayNo', e.target.value)} />
                  </div>

                  {/* Row 7: Delivery Mode */}
                  <div className="pi-field-row" style={{ gridTemplateColumns: '100px 1fr' }}>
                     <label className="pi-label">Delivery</label>
                     <select className="pi-select" value={doc.cnDetail.deliveryMode} onChange={e => handleNested('cnDetail', 'deliveryMode', e.target.value)}>
         ...
         <div className="pi-bottom-grid">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                     ...
                     {doc.terms.map((t, idx) => (
                        <div key={idx} className="pi-terms-row" style={{ alignItems: 'flex-start' }}>
                           <input className="pi-input" style={{ fontWeight: 600 }} placeholder="Title" value={t.title} onChange={e => { const nt = [...doc.terms]; nt[idx].title = e.target.value; setDoc({ ...doc, terms: nt }); }} />
                           <div style={{ flex: 1, display: 'flex', gap: '0.5rem' }}>
                              <textarea className="pi-textarea" rows={1} placeholder="Detail" value={t.detail} onChange={e => { const nt = [...doc.terms]; nt[idx].detail = e.target.value; setDoc({ ...doc, terms: nt }); }} />
                              <button className="pi-remove-btn" onClick={() => { const nt = [...doc.terms]; nt.splice(idx, 1); setDoc({ ...doc, terms: nt }); }}><X size={14} /></button>
                           </div>
                        </div>
                     ))}
                  </div>
                  <button className="pi-add-notes-btn" onClick={() => setDoc({ ...doc, terms: [...doc.terms, { title: '', detail: '' }] })}>+ Add Notes</button>
               </div>
               <div className="pi-divider" />
            ...
            <div className="pi-right-bottom">
               ...
               <div className="pi-add-charge-link" onClick={() => setAdditionalChargeModal(true)}>Add Additional Charge</div>
               ...
               <div className="pi-modifier-row">
                  ...
                  <div style={{ display: 'flex', gap: '4px', flex: 1, justifyContent: 'flex-end' }}>
                     <select className="pi-modifier-select" value={doc.tcs.mode} onChange={e => handleNested('tcs', 'mode', e.target.value)}><option>+</option><option>-</option></select>
                     <input className="pi-modifier-input" style={{ width: '80px' }} value={doc.tcs.value} onChange={e => handleNested('tcs', 'value', e.target.value)} />
                     <select className="pi-modifier-unit" value={doc.tcs.unit} onChange={e => handleNested('tcs', 'unit', e.target.value)}><option>%</option><option>Rs</option></select>
                  </div>
               </div>
               ...
               <div className="pi-modifier-row">
                  ...
                  <div style={{ display: 'flex', gap: '4px', flex: 1, justifyContent: 'flex-end' }}>
                     <select className="pi-modifier-select" value={doc.discount.mode} onChange={e => handleNested('discount', 'mode', e.target.value)}><option>-</option><option>+</option></select>
                     ...
                     <select className="pi-modifier-unit" value={doc.discount.unit} onChange={e => handleNested('discount', 'unit', e.target.value)}><option>Rs</option><option>%</option></select>
                  </div>
               </div>
               ...
               <div className="pi-totals-row" style={{ borderBottom: 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                     ...
                     <label className="pi-switch" style={{ width: '44px', height: '22px', position: 'relative', display: 'inline-block' }}>
                        ...
                        <span style={{ position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0, background: doc.roundOff ? '#10b981' : '#ccc', transition: '.4s', borderRadius: '34px' }}>
                           <span style={{ position: 'absolute', content: '""', height: '16px', width: '16px', left: '4px', bottom: '3px', background: 'white', transition: '.4s', borderRadius: '50%', transform: doc.roundOff ? 'translateX(20px)' : 'translateX(0)' }}></span>
                        </span>
                     </label>
                  </div>
               ...
               <div className="pi-grand-total" style={{ background: '#fffbeb', padding: '1rem', borderRadius: '8px', margin: '1rem 0' }}>
                  <span className="pi-grand-label" style={{ fontWeight: 800, fontSize: '1.2rem' }}>Grand Total</span>
                  <span className="pi-grand-value" style={{ fontWeight: 900, fontSize: '1.5rem', color: '#1e293b' }}>₹ {doc.grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
               </div>

               <div className="pi-words-row" style={{ border: 'none' }}>
                  <div className="pi-words-label">Total in words</div>
                  <div className="pi-words-value" style={{ textTransform: 'uppercase', color: '#64748b', fontSize: '0.8rem' }}>{numberToWords(doc.grandTotal)}</div>
               </div>

               <div className="pi-smart-box" style={{ marginTop: '1rem', background: '#f8fafc', padding: '1rem', borderRadius: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                     <span className="pi-smart-label">Smart Suggestion</span>
                     <button style={{ background: '#10b981', color: 'white', border: 'none', width: '20px', height: '20px', borderRadius: '4px' }}>-</button>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginTop: '1rem' }}>
                     <input type="checkbox" id="emailChkMain" checked={doc.shareEmail} onChange={e => setDoc({ ...doc, shareEmail: e.target.checked })} />
                     <label htmlFor="emailChkMain" className="pi-label" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: '#10b981' }}>Share on Email <Mail size={14} /></label>
                  </div>
               </div>
            </div>
         </div>
         ...
         <div className="pi-action-bar" style={{ background: 'white', padding: '1.5rem 2rem', borderRadius: '12px', marginTop: '2rem' }}>
            <div className="pi-action-left">
               <button className="pi-btn pi-btn-ghost" onClick={() => navigate('/documents')} style={{ padding: '0.75rem 2rem' }}>Back</button>
            </div>
            <div className="pi-action-right" style={{ display: 'flex', gap: '1rem' }}>
               <button className="pi-btn pi-btn-print" style={{ background: 'white', color: '#10b981', border: '2px solid #10b981', padding: '0.75rem 2rem' }} onClick={() => handleSave(true)} disabled={isSubmitting}>
                  <Printer size={18} /> Save & Print
               </button>
               <button className="pi-btn pi-btn-save" style={{ background: '#10b981', color: 'white', border: 'none', padding: '0.75rem 2rem' }} onClick={() => handleSave(false)} disabled={isSubmitting}>
            ...
            <div className="pi-modal-overlay" style={{ zIndex: 1000 }}>
               <div className="pi-card" style={{ width: '400px', padding: '2rem' }}>
                  <div className="pi-card-header" style={{ border: 'none', padding: '0 0 1.5rem 0', background: 'none' }}>
                     <h3 className="pi-title">Add Additional Charge</h3>
                     <button className="pi-reset-btn" onClick={() => setAdditionalChargeModal(false)}><X size={18} /></button>
                  </div>
                  <div className="pi-card-body" style={{ padding: 0 }}>
                     <div className="pi-field-row" style={{ gridTemplateColumns: '130px 1fr' }}>
                     ...
                     <div className="pi-field-row" style={{ gridTemplateColumns: '130px 1fr' }}>
                        ...
                        <input type="number" className="pi-input" value={doc.additionalCharge} onChange={e => setDoc({ ...doc, additionalCharge: e.target.value })} />
                     </div>
                     <button className="pi-btn pi-btn-save w-full" style={{ marginTop: '1.5rem', background: '#10b981', color: 'white' }} onClick={() => setAdditionalChargeModal(false)}>Apply Charge</button>
                  </div>
               </div>
            </div>
         )}
